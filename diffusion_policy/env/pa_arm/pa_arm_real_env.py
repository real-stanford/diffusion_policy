...
class PAArmRealEnv(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = spaces.Dict({
            "replicator_rgb": spaces.Box(
                low=0, high=1, shape=(3, render_size, render_size), dtype=np.float32
            ),
            "replicator_01_rgb": spaces.Box(
                low=0, high=1, shape=(3, render_size, render_size), dtype=np.float32
            ),
            "joint_positions": spaces.Box(
                low=-ws, high=ws, shape=(4,), dtype=np.float32
            ),
        })
        self.action_space = spaces.Box(low=-ws, high=ws, shape=(4,), dtype=np.float64)
